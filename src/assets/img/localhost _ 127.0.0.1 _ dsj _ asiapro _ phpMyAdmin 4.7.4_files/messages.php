...
PMA_messages['strDatabaseRenameToSameName'] = "数据库名称已存在. 请更换名字再试";
...
PMA_messages['strConfirmDeleteResults'] = "删除在 %s 表中找到的所有记录吗？";
...
PMA_messages['strMysqlAllowedValuesTipTime'] = "MySQL 可接受滑动条上不可选的其他值；如果需要直接键入这些值";
PMA_messages['strMysqlAllowedValuesTipDate'] = "MySQL 可接受日期选择器无法选取的其他值；如果需要直接键入这些值";
...
PMA_messages['strNoLocalStorage'] = "查看你的浏览器保存数据时出现错误，因此一些功能可能不能正常工作。造成这个问题的原因可能是你的浏览器不支持保存数据或者浏览器能够保存的数据达到极限。另外对于Firefox浏览器，这个问题可能是由于浏览器存储数据被损坏导致，清理浏览器的“离线网站数据”可能会解决这个问题。对于Safari浏览器，这个问题通常是由于“隐私浏览模式”导致的。";
...
PMA_messages['strExtrWeak'] = "极弱";
PMA_messages['strVeryWeak'] = "很弱";
PMA_messages['strWeak'] = "弱";
PMA_messages['strGood'] = "优";
PMA_messages['strStrong'] = "强";
